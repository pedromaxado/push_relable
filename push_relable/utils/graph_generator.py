...
def generate_digraph(n, m):
    nodes = [x for x in range(int(n))]
    num_edges = m

    approach = random_walk

    # Run
    graph = approach(nodes, num_edges, False, False, True)

    graph.sort_edges()
    edges = graph.edges

    for i, k in enumerate(edges):
        edges[i] = (k[0], k[1], random.randint(1, 50))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-u', '--ugly', action='store_true',
                        help='show ugly graph (one edge per line, separated by spaces)')
    parser.add_argument('-k', '--weight', action='store_true',
                        help='include random weights')
